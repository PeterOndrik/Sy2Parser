...
def showNode(handle, node, callbackContext):
    # gets pointer (pNode) from address (node)
	pNode = cast(node, POINTER(T_Sy2Node)).contents
	print("{}: {}".format(T_Sy2NodeType(pNode.type).name, pNode.value))

	newNode = T_Sy2Node()
	status = sy2dll.sy2ReadNext(handle, byref(newNode))
	while newNode.depth > pNode.depth:
...
sy2file.value = path.abspath("../../../Test/In/test.sy2").encode()
